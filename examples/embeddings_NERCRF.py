...
def main():
    ...
    embedd_dim = 300
    embedd_dict = embeddings.GloveEmbedding('common_crawl_840', d_emb=embedd_dim, show_progress=True)
    ...
    def construct_word_embedding_table():
        ...
        for word, index in word_alphabet.items():
            print(word)
            found = embedd_dict.emb(word)
            if None not in found:
                embedding = found
            else:
                found = embedd_dict.emb(word.lower())
                if None not in found:
                    embedding = found
                else:
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        with torch.no_grad():
            ...
            if dev_f1 < f1:
                ...
                torch.save(network, 'examples/nercrf_network.pt')
